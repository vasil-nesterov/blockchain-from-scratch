...
fn build_valid_chain(n: u64) -> Vec<Header> {
    ...
    for _ in 1..n {
        let new_header = chain.last().unwrap().child(2);
...
fn build_an_invalid_chain() -> Vec<Header> {
    ...
    let header2 = genesis.child(2);
    let header3 = header2.child(2);

    vec![genesis, header3, header2]
}
...
fn build_forked_chain() -> (Vec<Header>, Vec<Header>) {
    ...
    (
        vec![genesis, genesis.child(2)],
        vec![genesis, genesis.child(3)],
    )
